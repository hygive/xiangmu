<template>
  <div class="hello">
    <p v-for="(item,index) of options" :key="index" class="test">
      <label class="myRadio">
        <input :typename="typename" :id="id" type="radio" :checked="checkvalue.indexOf(item)>-1"  :value="item" :name="name"  @change="updataInp"/>
        <span>{{item}}</span>
      </label>
    </p>
  </div>
